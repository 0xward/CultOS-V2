...
- viralScore: integer 1-100, honest assessment of cultural propagation potential.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  for (const model of MODELS) {
    try {
      ...
      console.info(`[Oracle] Success via: ${model} — ticker=${result.ticker} viralScore=${result.viralScore}`);
